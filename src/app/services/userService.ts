...
export class UserService extends BasicService<User> {
  ...
  public login(entity: Login): Observable<string> {
    return this.http.post(this.Url + 'login/', entity, {headers: this.headers})
      .pipe(map((data: string) => data));
  }

  public register(entity: Register): Observable<string> {
    return this.http.post(this.Url + 'register/', entity, {headers: this.headers})
      .pipe(map((data: string) => data));
  }

  public inviteUsersToGroup(entity: GroupInvite): Observable<string> {
    return this.http.post(this.Url + 'inviteToGroup/', entity, {headers: this.headers})
      .pipe(map((data: string) => data));
  }

  public registerCode(code: string): Observable<string> {
    return this.http.get(this.Url + 'registerCode/' + code, {headers: this.headers})
      .pipe(map((data: string) => data));
  }

  public inviteCode(code: string): Observable<string> {
    return this.http.get(this.Url + 'inviteCode/' + code, {headers: this.headers})
      .pipe(map((data: any) => data));
  }

  public checkAuth(): Observable<true> {
    return this.http.post(this.Url + 'checkAuth/', {headers: this.headers})
      .pipe(map(() => true));
  }
