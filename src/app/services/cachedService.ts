...
export class CachedService<T extends Base> extends BasicService<T> {
    ...
    public getList(filter: any = null, sort: any = null, limit: number = null, skip: number = null): Observable<T[]> {
        return super.getUpdateDateOfTable()
            .pipe(switchMap((date: Moment) => {
                if (date > this.latestUpdate) {
                    this.latestUpdate = date;
                    return super.getList(filter, sort, limit)
                        .pipe(switchMap(
                            (data: T[]) => {
                                this.cache.clear();
                                data.forEach(d => {
    ...
    public getCount(filter: any = null, sort: any = null, limit: number = null, skip: number = null): Observable<number> {
        ...
        //     .pipe(switchMap((date: Moment) => {
        //         if (date > this.latestUpdate) {
                    return super.getCount(filter, sort, limit)
                        .pipe(switchMap(
                            (count: number) => {
                                //this.cache.clear();
                                return of(count);
